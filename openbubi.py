...
class BubiUser:
    ...
    def getSubscriptionInfo(self):
        linkUrl = json.loads(self.getPaymentLinks())[
            "paymentlinks"][0]["link_url"]
        contents = requests.get(linkUrl).text
        for i in contents.splitlines():
            if "Érvényeség" in i:
                endOfSubscription = i.strip()
                endOfSubscription = endOfSubscription.replace(
                    "Érvényeség vége: ", "")
        subscriptionInfo = {}
        if "havi" in contents:
    ...
    def getNearestStations(self, lat, lon):
        ...
        for i in range(len(stations)):
            ...
            difference = math.sqrt(
                abs(currentStation["lat"] - lat)**2 + abs(currentStation["lon"] - lon)**2)
            differences.update({currentStation["name"]: difference})
        sortedDifferences = dict(
